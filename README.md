...
## 📦 Установка и использование инсталлятора / PAC

После установки с помощью инсталлятора (или ручной распаковки) установщик может настроить системный PAC (AutoConfigURL) для текущего пользователя.

- Инсталлятор копирует `zapret_proxy.pac` в папку установки и выставляет в реестре Windows (HKCU) ключ `AutoConfigURL` со значением `file://{app}\zapret_proxy.pac`.
- Это заставит приложения, использующие системную настройку авто-настройки прокси (PAC), направлять трафик согласно правилам PAC.

Важно:
- PAC маршрутизирует только трафик к доменам, указанным в `src/config.json` (`enabled_services`) через локальный прокси (по умолчанию `127.0.0.1:8080`). Остальной трафик идёт напрямую.
- Некоторые нативные приложения могут игнорировать системные PAC/прокси (они будут обходиться напрямую). В таких случаях потребуется либо настроить приложение вручную, либо использовать системный level метод (WinDivert/driver) — это выходит за рамки PAC-интеграции.

Откат изменений (удаление PAC):

1. Через деинсталлятор — если вы устанавливали через `ZapretProxySetup.exe`, при удалении инсталлятор удалит запись `AutoConfigURL` из `HKCU\Software\Microsoft\Windows\CurrentVersion\Internet Settings`.
2. Ручной откат:
...
Если вы хотите временно использовать PAC без установки — можно указать `Auto-config URL` в настройках сети Windows и выбрать локальный путь к `zapret_proxy.pac`.

## 🔒 Список фильтруемых сервисов (по умолчанию)

Фильтрация по умолчанию включена для следующих сервисов (домены указаны в `src/config.json`):
...
Чтобы изменить набор фильтруемых сервисов — отредактируйте `src/config.json` (поле `enabled_services`) и перезапустите прокси/приложение.

## 📥 Релиз и установщик

Готовый установщик автоматически собран и прикреплён к релизу v0.1.0.

- Страница релиза: https://github.com/plato1nnnnnnnnnnnnn/zapret-edit/releases/tag/v0.1.0
- Прямой скачиваемый установщик: https://github.com/plato1nnnnnnnnnnnnn/zapret-edit/releases/download/v0.1.0/ZapretProxySetup.exe

Если нужно — могу добавить автоматический шаг в CI, который будет сам создавать/прикреплять установщик к релизу.
